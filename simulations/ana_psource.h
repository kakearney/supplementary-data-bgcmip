...
#elif defined BERING_10K
        DO is=1,Nsrc(ng)
          SOURCES(ng)%Qbar(is)=800000._r8/Nsrc(ng)
        END DO
#else
...
# elif defined BERING_10K 
        ! KAK: Is there a way to turn on momentum point sources without tracer point 
        ! sources at compile time?  I'm including this code for now to get around that, 
        ! but may need to revisit this down the road.  Reminder: these point sources 
        ! are used to force Bering Strait flow.
        DO is=1,Nsrc(ng)
