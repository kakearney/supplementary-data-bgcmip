% bgcmip_paper_final.m
%
% This is a cleaned-up version of previous analysis scripts that contains
% only the code necessary to reproduce all figures and numbers in the final
% BGCMIP manuscript.  See bgcmip_paper2.m for a messier version with a lot
% more intermediate fiddling, figure versions for slide presentations, etc.
%
% Scripts run prior to running this:
%
% readbgcmetrics.m: reads output from specific locations into tables, saved
% in the nstation x nvariant structure Metrics with fields corresponding to
...
% 4. "bury_CFS_loop"
% 5. "burynoinfauna_CFS"
% 6. "burynoinfauna_CFS_loop";
%
% readnbudget.m:  Reads nitrogen budget data into Matlab graph objects with
% attached timeseries of node biomass and edge flux.  Data saved to
% nbudget*.mat files, one file per spatial polygon.
...
% 
% Etop(1) = ncstruct(tilename, struct('lat', latscs, 'lon', lonscs(1,:)), 'lat', 'lon', 'z');
...
% Update March 2022, OpenDAP read is failing so downloaded to local
% geotiffs instead

Etop = struct;
[Etop(1).z,rtmp] = readgeoraster('/Volumes/LaCie2023/ETOPO2022/Bering_left.tiff');
[Etop(1).lat, Etop(1).lon] = geographicGrid(rtmp);
[Etop(2).z,rtmp] = readgeoraster('/Volumes/LaCie2023/ETOPO2022/Bering_right.tiff');
[Etop(2).lat, Etop(2).lon] = geographicGrid(rtmp);

Etop(1).lon = wrapTo360(cat(2, Etop.lon));
Etop(1).lat = cat(2, Etop.lat);
Etop(1).z   = cat(2, Etop.z);
Etop = Etop(1);

[P.xtop, P.ytop] = projfwd(proj, Etop.lat, Etop.lon);
...
[lonc, latc] = polycenter(rmfield(Bsierp, ["X","Y"]));
[P.xbsierplbl, P.ybsierplbl] = projfwd(proj, latc, lonc);
...
[lonc, latc] = polycenter(rmfield(Strata, ["X","Y"]));
[P.xstratalbl, P.ystratalbl] = projfwd(proj, latc, lonc);
...
colormap(adjustcol(colors{'gray','rgb'}, 1.0));
...
str = arrayfun(@(X) "\it "+string(wraptext(X.name, 10)), Bsierp, 'UniformOutput', false);
h.lblreg = text(P.xbsierplbl, P.ybsierplbl,str, 'horiz', 'center', 'color', adjustcol(colors{'yellow','rgb'}, -0.3), 'fontsize', 6, 'fontname', 'Atkinson Hyperlegible');
h.lblreg(13).Position = h.lblreg(13).Position + [10 4 0]*10000; % manual tweak for legibility
...
pltvars = {...
     "npp",         'NPP (mmol N m^{-2} d^{-1})'   60  
    ["NO3s" "NO3b"] 'NO_3 (mmol N m^{-3})' NaN
    ["NH4s" "NH4b"] 'NH_4 (mmol N m^{-3})'      NaN
     "zprod_micro"  'Microzoo. prod. (mmol N m^{-2} d^{-1})' 40
     "zprod_meso"   'Mesozoo. prod. (mmol N m^{-2} d^{-1})' NaN
...
for iplt = 1:height(pltvars)
    ...
    ylabel(h.ax(1,1), pltvars.lname{iplt}, 'fontsize', 10);
...
h.fig.Position = [361  361  887.76 144]; % matching to biorender canvas
...
%% ... plot log-scaled biomass with consolidated groups (P/Z x loop/primary panels)
